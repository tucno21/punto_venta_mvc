...
async function generarDataTable() {
  ...
  let newData = {
    headings: ["#", "Código", "Tipo", "Descripción", "Estado", "Acciones"],
    data: data.map((item) => {
      return [
        item.orden.toString(),
...
function botonCrear() {
  btnCrear.addEventListener("click", () => {
    limpiarErrrorInput([inputCodigo, inputDescripcion, inputTipo]);
...
async function enviarCrearFormulario() {
  ...
  if (dRes.status) {
    ...
    limpiarErrrorInput([inputCodigo, inputDescripcion, inputTipo]);
    modalInputs.hide();
    toastPersonalizado("success", "Tabla parametrica Registrado");
  } else {
...
async function enviarEditarFormulario() {
  ...
  if (dRes.status) {
    ...
    limpiarErrrorInput([inputCodigo, inputDescripcion, inputTipo]);
    modalInputs.hide();
    toastPersonalizado("success", "Tabla parametrica editado");
  } else {
    mensajeErrorInput(inputCodigo, dRes.data.codigo);
    mensajeErrorInput(inputDescripcion, dRes.data.descripcion);
    mensajeErrorInput(inputTipo, dRes.data.tipo);
...
async function botonEliminar(url) {
  ...
  if (accept) {
    ...
    if (data.status) {
      generarDataTable();
      toastPersonalizado("success", "Tabla parametrica eliminado");
    }
  }
}
