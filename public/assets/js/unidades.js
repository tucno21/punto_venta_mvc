...
async function generarDataTable() {
  ...
  let newData = {
    headings: ["#", "Nombre", "Abreviatura", "Estado", "Acciones"],
    data: data.map((item) => {
      return [
        item.orden.toString(),
        item.descripcion,
...
function botonCrear() {
  btnCrear.addEventListener("click", () => {
    limpiarErrrorInput([inputName, inputCodigo]);
...
async function enviarCrearFormulario() {
  ...
  data.append("descripcion", inputName.value);
  ...
  if (dRes.status) {
    ...
    limpiarErrrorInput([inputName, inputCodigo]);
    modalInputs.hide();
    toastPersonalizado("success", "Unidad Registrado");
  } else {
    mensajeErrorInput(inputName, dRes.data.descripcion);
...
async function botonEditar(url) {
  ...
  inputName.value = data.data.descripcion;
...
async function enviarEditarFormulario() {
  ...
  if (dRes.status) {
    ...
    limpiarErrrorInput([inputName, inputCodigo]);
    modalInputs.hide();
    toastPersonalizado("success", "Unidad editado");
  } else {
    mensajeErrorInput(inputName, dRes.data.descripcion);
    mensajeErrorInput(inputCodigo, dRes.data.codigo);
  }
