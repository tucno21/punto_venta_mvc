...
async function enviarCrearFormulario() {
  ...
  data.append("tipo_comprobante", inputComprobante.value);
  data.append("serie", inputSerie.value);
  data.append("correlativo", inputCorrelativo.value);
  data.append("tipo", inputTipo.value);
  ...
  if (dRes.status) {
    ...
    modalInputs.hide();
    toastPersonalizado("success", "Serie - Correlativo Registrado");
  } else {
    mensajeErrorInput(inputComprobante, dRes.data.tipo_comprobante);
    mensajeErrorInput(inputSerie, dRes.data.serie);
    mensajeErrorInput(inputCorrelativo, dRes.data.correlativo);
    mensajeErrorInput(inputTipo, dRes.data.tipo);
  }
}
...
async function botonEditar(url) {
  ...
  inputComprobante.value = data.data.tipo_comprobante;
  inputSerie.value = data.data.serie;
  inputCorrelativo.value = data.data.correlativo;
  inputTipo.value = data.data.tipo;
...
async function enviarEditarFormulario() {
  ...
  if (dRes.status) {
    ...
    modalInputs.hide();
    toastPersonalizado("success", "Serie - Correlativo editado");
  } else {
    mensajeErrorInput(inputComprobante, dRes.data.tipo_comprobante);
    mensajeErrorInput(inputSerie, dRes.data.serie);
    mensajeErrorInput(inputCorrelativo, dRes.data.correlativo);
...
async function botonEliminar(url) {
  ...
  if (accept) {
    ...
    if (data.status) {
      generarDataTable();
      toastPersonalizado("success", "Serie - Correlativo eliminado");
    }
  }
}
