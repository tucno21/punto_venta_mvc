...
const inputCodigoAfectacion = document.querySelector("#inputCodigoAfectacion");
const inputNombreAfectacion = document.querySelector("#inputNombreAfectacion");
const inputTipoAfectacion = document.querySelector("#inputTipoAfectacion");
  ...
  let newData = {
    ...
    data: data.map((item) => {
      return [
        ...
        item.descripcion,
        item.codigo_afectacion.toString(),
        item.nombre_afectacion,
...
function botonCrear() {
  btnCrear.addEventListener("click", () => {
    limpiarErrrorInput([
      inputCodigo,
      inputDescripcion,
      inputCodigoAfectacion,
      inputNombreAfectacion,
      inputTipoAfectacion,
    ]);
...
async function enviarCrearFormulario() {
  ...
  data.append("codigo_afectacion", inputCodigoAfectacion.value);
  data.append("nombre_afectacion", inputNombreAfectacion.value);
  data.append("tipo_afectacion", inputTipoAfectacion.value);
  ...
  if (dRes.status) {
    ...
    modalInputs.hide();
    toastPersonalizado("success", "Tipo de Afectación Registrado");
  } else {
    mensajeErrorInput(inputCodigo, dRes.data.codigo);
    mensajeErrorInput(inputDescripcion, dRes.data.descripcion);
    mensajeErrorInput(inputCodigoAfectacion, dRes.data.codigo_afectacion);
    mensajeErrorInput(inputNombreAfectacion, dRes.data.nombre_afectacion);
    mensajeErrorInput(inputTipoAfectacion, dRes.data.tipo_afectacion);
  }
}
...
async function botonEditar(url) {
  ...
  inputCodigoAfectacion.value = data.data.codigo_afectacion;
  inputNombreAfectacion.value = data.data.nombre_afectacion;
  inputTipoAfectacion.value = data.data.tipo_afectacion;
...
async function enviarEditarFormulario() {
  ...
  if (dRes.status) {
    ...
    modalInputs.hide();
    toastPersonalizado("success", "Tipo de Afectación editado");
  } else {
    mensajeErrorInput(inputCodigo, dRes.data.codigo);
    mensajeErrorInput(inputDescripcion, dRes.data.descripcion);
    mensajeErrorInput(inputCodigoAfectacion, dRes.data.codigo_afectacion);
...
async function botonEliminar(url) {
  ...
  if (accept) {
    ...
    if (data.status) {
      generarDataTable();
      toastPersonalizado("success", "Tipo de Afectación eliminado");
    }
  }
}
