...
async function generarDataTable() {
  ...
  let newData = {
    headings: ["#", "Categoria", "Estado", "Acciones"],
    data: data.map((item) => {
      return [item.orden.toString(), item.nombre, item.estado, item.actions];
    }),
  };
...
async function enviarCrearFormulario() {
  ...
  data.append("nombre", inputName.value);
  ...
  if (dRes.status) {
    ...
    limpiarErrrorInput([inputName]);
    modalInputs.hide();
    toastPersonalizado("success", "Categoria Registrado");
  } else {
    mensajeErrorInput(inputName, dRes.data.nombre);
  }
}
...
async function botonEditar(url) {
  ...
  inputName.value = data.data.nombre;
...
async function enviarEditarFormulario() {
  ...
  if (dRes.status) {
    ...
    limpiarErrrorInput([inputName]);
    modalInputs.hide();
    toastPersonalizado("success", "Categoria editado");
  } else {
    mensajeErrorInput(inputName, dRes.data.nombre);
  }
}
...
async function botonEliminar(url) {
  ...
  if (accept) {
    ...
    if (data.status) {
      generarDataTable();
      toastPersonalizado("success", "Unidad eliminado");
    }
  }
}
