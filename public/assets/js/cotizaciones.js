...
const inputTiempoCotizacion = document.querySelector("#inputTiempoCotizacion");
//complemento
...
//carrito de compras
const tablaCotizaciones = document.querySelector("#tablaventas");
...
function cargarEventListeners() {
  ...
  tablaCotizaciones.addEventListener("click", eliminarProducto);
  //cambiar precio de producto
  tablaCotizaciones.addEventListener("change", cambioCantPrecioProducto);
  //ingreso de datos de cuota
  cuotasContainer.addEventListener("change", agregarCuotasArray);

  document.addEventListener("DOMContentLoaded", () => {
    productosCarrito =
      JSON.parse(localStorage.getItem("productoscarritoscotizaciones")) || [];
    carritoHTML();
    menuTipoComprobante();
    menuMonedas();
...
function generarVenta() {
  btnEnviarVentas.addEventListener("click", async (e) => {
    ...
    if (inputTiempoCotizacion.value == "") {
      toastPersonalizado("error", "Ingrese el tiempo de Oferta");
      return;
    }
    ...
    data.append("tiempo", inputTiempoCotizacion.value);
    ...
    data.append("forma_pago", "Contado");
    ...
    if (dRes.status) {
      ...
      toastPersonalizado("error", "No se registro la venta");
    }
  });
}
...
function eliminarLocalStorage() {
  //limpiar carrito
  productosCarrito = [];
  localStorage.removeItem("productoscarritoscotizaciones");
  carritoHTML();
}
