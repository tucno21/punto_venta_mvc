...
async function generarDataTable() {
  ...
  let newData = {
    headings: ["#", "Código", "Descripcion", "Estado", "Acciones"],
    data: data.map((item) => {
      return [
        item.orden.toString(),
        item.codigo,
...
function botonCrear() {
  btnCrear.addEventListener("click", () => {
    limpiarErrrorInput([inputCodigo, inputDescripcion]);
...
async function enviarCrearFormulario() {
  ...
  if (dRes.status) {
    ...
    limpiarErrrorInput([inputCodigo, inputDescripcion]);
    modalInputs.hide();
    toastPersonalizado("success", "Tipo de documento Registrado");
  } else {
...
async function enviarEditarFormulario() {
  ...
  if (dRes.status) {
    ...
    limpiarErrrorInput([inputCodigo, inputDescripcion]);
    modalInputs.hide();
    toastPersonalizado("success", "Tipo de documento editado");
  } else {
    mensajeErrorInput(inputCodigo, dRes.data.codigo);
    mensajeErrorInput(inputDescripcion, dRes.data.descripcion);
  }
...
async function botonEliminar(url) {
  ...
  if (accept) {
    ...
    if (data.status) {
      generarDataTable();
      toastPersonalizado("success", "Tipo de documento eliminado");
    }
  }
}
